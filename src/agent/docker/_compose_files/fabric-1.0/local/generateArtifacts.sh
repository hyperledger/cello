...
echo $CHANNEL_NAME
TOP_DIR=$PWD
CRYPTO_PATH=$TOP_DIR/crypto-config
ARTIFACT_PATH=$TOP_DIR/channel-artifacts
echo
...
function replacePrivateKey () {
	...
	cp docker-compose-e2e-template.yaml cluster-7.yml

	CURRENT_DIR=$PWD
	cd crypto-config/peerOrganizations/org1.example.com/ca/
	PRIV_KEY=$(ls *_sk)
	cd $CURRENT_DIR
	sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" cluster-7.yml
